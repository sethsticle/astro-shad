...
2. **`background` is the one named slot** — an absolutely-positioned layer behind the content (`relative isolate` on the section, `absolute inset-0 -z-10` wrapper), empty by default, reserved for the effects pass (imagery, vignettes, gradient masks). Media placeholders inside split layouts are token-styled surfaces the consumer swaps for their own `<img>`/`<video>`.
